...
The network is made up of a series of asset pairs forming a specified **Market**. If an asset pair does not yet have a *market* it can be created by anyone exposing a public reachable endpoint and providing liquidity. 

Each *market* holds reserves of a **base asset** and a **quote asset**. Anyone can become a **liquidity provider** and contribute with reserves in a non-custodial manner running an always-on endpoint to process trading requests. This is different than buying or selling; it requires depositing an equivalent value of both base and quote assets and select an automated market making strategy. 
